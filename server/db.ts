...
export async function getUserStats(userId: number) {
  ...
  // Most used skins TOP 3
  const mostUsedSkins = await db
    .select({
      skinKey: transformHistory.skinKey,
      count: sql<number>`COUNT(*) as count`,
    })
    .from(transformHistory)
    .where(eq(transformHistory.userId, userId))
    .groupBy(transformHistory.skinKey)
    .orderBy(sql`count DESC`)
    .limit(3);

  // Favorite skins count
  ...
  const recentHistory = await db
    .select({
      id: transformHistory.id,
      skinKey: transformHistory.skinKey,
      originalText: transformHistory.originalText,
      transformedText: transformHistory.transformedText,
      createdAt: transformHistory.createdAt,
    })
    .from(transformHistory)
    .where(eq(transformHistory.userId, userId))
    .orderBy(desc(transformHistory.createdAt))
