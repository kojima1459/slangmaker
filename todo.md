...
## 設定画面削除とトップ画面API入力欄追加、アプリ名変更

- [ ] SNS共有機能拡張のチェックポイント作成
- [ ] Settings.tsxファイルを削除
- [ ] ヘッダーから設定ボタンを削除
- [ ] App.tsxから設定ルートを削除
- [ ] ヘッダーのレイアウトをバランス良く再配置
- [ ] トップ画面にAPIキー入力欄を追加（localStorage保存）
- [ ] Gemini APIキー取得ガイドリンクを追加
- [ ] アプリ名を「AI言い換えメーカー」から「AIスラングメーカー」に変更（全ファイル）
- [ ] 多言語翻訳の徹底確認：スキン名（全13種類）
- [ ] 多言語翻訳の徹底確認：スキン説明文
- [ ] 多言語翻訳の徹底確認：UI要素すべて
- [ ] 多言語翻訳の徹底確認：エラーメッセージ
- [ ] 最終チェックポイント作成
- [ ] シェア時のURLを https://slang-maker.manus.space に統一
