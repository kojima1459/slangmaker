...
- [ ] スキン名を多言語対応
- [ ] ボタンテキストを多言語対応
- [ ] 詳細設定のラベルとプレースホルダーを多言語対応
- [ ] エラーメッセージを多言語対応

### Phase 4: Settings.tsxを多言語対応
- [ ] 設定ページのタイトルとラベルを多言語対応
- [ ] ボタンテキストを多言語対応
- [ ] 成功・エラーメッセージを多言語対応
...
### Phase 1: Reader.tsxにフィードバックUIを実装
- [x] Reader.tsxに「👍良い」「👎悪い」ボタンを追加
- [x] フィードバック送信時のトースト通知を実装
- [x] フィードバック送信後のボタン状態管理（既に送信済みの場合は無効化）
- [x] transform APIかhistoryIdを返すように修正
- [x] Home.tsxでhistoryIdをsessionStorageに保存

### Phase 2: フィードバック分析ダッシュボードを作成
- [x] FeedbackDashboard.tsxページを作成
- [x] スキン別のフィードバック統計を表示（良い/悪いの割合）
- [x] 評価の低いスキンを強調表示（赤色のボーダーと背景）
- [x] App.tsxに/feedbackルートを追加
- [x] 管理者用のナビゲーションリンクを追加（Settings.tsxに管理者セクションを追加）

### Phase 3: スキン自動改善機能を実装
- [x] 低評価スキンのプロンプト改善APIを実装（feedback.improveSkin）
- [x] LLMを使ってプロンプトを自動生成（invokeLLM + JSON schema）
- [x] 改善提案をプレビュー表示（モーダルで表示）
- [x] FeedbackDashboard.tsxに改善ボタンを追加
- [x] コピーボタンで改善内容をクリップボードにコピー
