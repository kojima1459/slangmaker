...
## Phase 14: カスタムスキン機能テスト（2025-12-09）

### テスト内容
- [x] カスタムスキン作成機能の実装確認
- [x] カスタムスキンの保存機能の確認（localStorage）
- [x] カスタムスキンでの変換テスト
- [x] カスタムスキンの削除機能の確認
- [x] ブラウザでの実際の動作確認
...
## Phase 18: スマホ問題修正（2025-12-09）

### 問題①: チュートリアルの内容が間違っている
- [x] 「APIキーを入力」ステップを削除（Manus Built-in LLM APIを使用しているため不要）
- [x] 「記事を貼り付け」を「テキストを貼り付け」に変更

### 問題②: LinkedInシェア時にサイトURLしか表示されない
- [x] LinkedInシェア機能を修正（X投稿時と同じように変換結果のテキストを含める）

### 問題③: ヘッダーのレイアウト崩れ
- [x] 履歴とXが重複して重なっている問題を修正
- [x] 言語切り替えボタンが見切れている問題を修正

### ブラウザテスト
- [ ] スマホでチュートリアルの表示を確認
- [ ] スマホでLinkedInシェア機能を確認
- [ ] スマホでヘッダーのレイアウトを確認
- [ ] チェックポイント作成

## Phase 19: FAQ翻訳機能修正とInstagramシェア機能実装（2025-12-09）

### タスク①: FAQページの翻訳機能修正
- [x] FAQ.tsxを確認して翻訳キーの使用状況を調査
- [x] i18n.tsにFAQ用の翻訳キーを追加（日本語・英語・中国語）
- [x] FAQ.tsxは既にuseTranslation()を使用して多言語対応済み
- [ ] ブラウザで各言語の表示を確認

### タスク②: Instagramシェア機能の追加
- [x] Reader.tsxにInstagramシェアボタンを追加
- [x] Instagram用のシェアURLフォーマットを実装（画像化機能への誘導）
- [x] アイコンとスタイルを他のSNSボタンと統一

### タスク③: SNS別最適サイズ選択機能の実装
- [x] SNS別の最適サイズを定義（X: 1200x675px, Instagram: 1080x1080px, Facebook: 1200x630px等）
- [x] ImageGenerator.tsxにサイズ選択UIを追加
- [x] 各SNS用の画像生成ロジックを実装
- [x] レイアウトを各サイズに最適化（横長は2カラム、縦長は1カラム）

### ブラウザテスト
- [ ] FAQページの多言語切り替えを確認
- [ ] Instagramシェアボタンの動作を確認
- [ ] 各SNSサイズの画像生成を確認
- [ ] チェックポイント作成

## Phase 20: トップページチュートリアル完全修正（2025-12-09）

### 問題点
- ステップ1「記事を貼り付け」→「テキストを貼り付け」に変更が必要
- ステップ3「APIキーを入力」→このステップ自体を削除が必要（Manus Built-in LLM APIを使用）

### タスク
- [x] i18n.tsのチュートリアル翻訳を修正（日本語・英語・中国語）
- [x] Tutorial.tsxのチュートリアルステップ配列を修正（APIキーステップを削除）
- [x] ステップ番号を再調整（5ステップ→4ステップ）
- [ ] ブラウザで確認
- [ ] チェックポイント作成

## Phase 21: ヘッダーSNSボタンと画像化機能の改善（2025-12-09）

### 問題①: ヘッダーのSNSシェアボタンが区別できない
- [x] スマホ表示時にXとLINEのアイコンが見分けにくい
- [x] アイコンをTwitterとMessageCircleに変更して明確に区別
- [x] title属性でツールチップを追加

### 問題②: 画像化機能のスマホ対応
- [x] スマホでの表示サイズはSNS別最適サイズ選択で対応済み
- [x] Web Share APIを使用してカメラロールにワンタッチで保存できる仕様に変更
- [x] 画像右下に「AIスラングメーカー」の透かしロゴを追加
- [x] 画像右下にサイトURL（slang-maker.manus.space）を追加
- [x] 透かしはopacity-60で控えめに設定
...
## Phase 22: スマホ版画像保存方法の改善

### 問題
- [ ] スマホでWeb Share APIを使用しても、カメラロールにワンタップで保存できない
- [ ] 共有シートから「画像を保存」を選ぶ必要があり、直感的ではない

### 解決策：長押し保存方式
- [x] 画像生成後、`<img>`タグで画像を表示
- [x] 「📱 画像を長押しして保存できます」というガイドメッセージを追加
- [x] iOS/Androidの両方で動作する長押し保存方式に変更
- [x] ユーザーフレンドリーなメッセージでわかりやすく案内（スマホ・PC両方）
- [x] アニメーション効果で目立たせる（animate-pulse）
- [ ] チェックポイント作成

## Phase 23: マーケティング素材作成（Manus 1兆トークンキャンペーン応募用）

### タスク①: X宣伝用文章3パターン作成
- [x] Manusキャンペーンの評価基準を確認（https://manus.im/ja/campaign/free-tokens）
- [x] バズり・ユーザー訴求を目的とした文章を3パターン作成
- [x] 280文字程度、#BuiltwithManus含む
- [x] 感情訴求・ロジカル訴求・キャッチーなフレーズを使用

### タスク②: 広告画像4枚作成
- [x] X投稿用の広告画像3枚（特徴・機能・面白さ訴求）
- [x] ロゴ画像1枚
- [ ] 日本語、ポップまたはリッチなデザイン
...
## Phase 24: Manus 1兆トークンキャンペーンバッジ追加

### タスク
- [x] キャンペーンバッジのデザインを設計（目立つ位置、アニメーション効果）
...
### フェーズ1: routers.tsへのセキュリティ機能統合
- [x] TransformServiceをrouters.tsに統合
- [x] レート制限ミドルウェアを追加
- [x] CSRF保護を追加
- [x] ロギングを追加
- [x] 統合後のユニットテスト実行（13/13テスト成功）

### フェーズ2: E2Eテスト実装
- [x] Playwrightのインストールとセットアップ
- [x] E2E_TESTS.mdに基づいてテストコード作成（19件）
- [x] 成功パステスト（7件）
- [x] エラーハンドリングテスト（5件）
- [x] ユーザーインタラクションテスト（5件）
- [x] E2Eテスト実行と検証（16/19テスト成功、84%）
...
### 失敗したテスト
- [x] TC-009: 文字数超過エラー（タイムアウト）
- [x] TC-011: APIタイムアウトエラー（ローディングインジケーター未検出）
- [x] TC-012: レート制限エラー（テキストエリア無効化）

### 修正作業
- [x] 失敗原因の分析
- [x] UI実装に合わせてテストケースを修正
- [x] 修正後のテスト実行
- [x] 全E2Eテストが成功することを確認（19/19テスト成功、100%）
- [ ] チェックポイント保存
...
- [x] Compare.tsxにSNSシェアボタンセクションを追加
- [x] X（Twitter）、LINE、Facebook、Instagram、LinkedInのシェアボタンを実装
- [x] Reader.tsxと同様のシェアロジックを適用
- [x] 比較結果のテキストをシェア用に整形
- [x] 動作確認（各SNSへのシェアが正常に動作するか）
- [x] チェックポイント保存（0819aed3）
- [x] GitHubにプッシュ（https://github.com/kojima1459/iikae-maker）
...
## 新機能: 7日間・30日間ランキングの追加

- [ ] stats.router.tsに7日間・30日間のランキング集計ロジックを追加
- [ ] Home.tsxにランキング期間切り替えタブを実装
- [ ] 24時間・7日間・30日間のランキングを表示
- [ ] 動作確認（各期間のランキングが正しく表示されるか）
- [ ] チェックポイント保存
- [ ] GitHubにプッシュ
- [ ] Google Driveにアップロード
